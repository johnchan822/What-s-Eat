    ...
    const autocompleteOptions = {
     ...
     const tempListDistances = useCallback(()=>{
        ...
        if (!isEmpty(directions) && isEmpty(selectedRestaurant?.directions) && !isEmpty(selectedRestaurant)){
            ...
            //寫入到tempList上面
              const isDuplicate = tempList.some(item => item.placeId === SelectedRestaurantDirections.placeId)
              if (!isDuplicate) {
            ...
            zoom={16}>   
                ...
                { 
                    ...
                    nearbySearch.map((item,index)=>{
                        const itemNameMore =  item.name.split("").length > 8  ? `${item.name.slice(0, 8) + "..."}` :  item.name ;
                ...
                onPlaceChanged={onPlaceChanged}>
                    ...
                    width: "80vw" }}>
                        ...
                        <div className="flex justify-between">
                            <div className="bg-white px-3 py-1 rounded-1 text-[14px] cursor-pointer"
                            style={{
